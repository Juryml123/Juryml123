// ============================================
// ANDROID DIAGNOSTICS & STORAGE UTILITY
// ============================================
// Check localStorage availability and provide fallbacks for Android Chrome
const StorageUtil = {
    isLocalStorageAvailable: false,
    isSessionStorageAvailable: false,
    memoryStore: {}, // Fallback in-memory storage
    debugMode: true,
    ...
    init() {
        ...
        try {
            ...
            console.warn('localStorage unavailable (Android private mode or quota exceeded)', e.message);
        ...
        // Log diagnostics
        if (this.debugMode) {
            console.log('%c📱 Portfolio Storage Diagnostics', 'color: #0b3b73; font-weight: bold; font-size: 14px');
            ...
            console.log('Fallback Memory Store:', '✅ Always Available');
        }
    },
    ...
    setItem(key, value) {
        try {
            if (this.isLocalStorageAvailable) {
                ...
                if (this.debugMode) console.log('Stored in memory:', key);
            }
        } catch (e) {
...
// Initialize storage utility on load
StorageUtil.init();

// ============================================
// IFRAME SYNC LISTENER (From CMS)
// ============================================
// Listen for data sync messages from the CMS iframe
window.addEventListener('message', function(event) {
    if (event.data && event.data.type === 'PORTFOLIO_DATA_SYNC') {
        const syncData = event.data.data;
        if (syncData) {
            console.log('%c📨 Received portfolio data sync from CMS', 'color: #0b3b73; font-weight: bold');
            
            // Update storage with synced data
            if (syncData.projects) StorageUtil.setItem('portfolioProjects', JSON.stringify(syncData.projects));
            if (syncData.bio) StorageUtil.setItem('portfolioBio', JSON.stringify(syncData.bio));
            if (syncData.skills) StorageUtil.setItem('portfolioSkills', JSON.stringify(syncData.skills));
            if (syncData.experience) StorageUtil.setItem('portfolioExperience', JSON.stringify(syncData.experience));
            if (syncData.certifications) StorageUtil.setItem('portfolioCertifications', JSON.stringify(syncData.certifications));
            if (syncData.media) StorageUtil.setItem('portfolioMedia', JSON.stringify(syncData.media));
            
            // Refresh page content if already loaded
            if (document.readyState === 'complete') {
                console.log('🔄 Refreshing content with synced data...');
                location.reload();
            }
        }
    }
...
(function() {
    ...
    if (isAndroid) {
        console.log('%c📱 Android Device Detected', 'color: #f59e0b; font-weight: bold; font-size: 14px');
        console.log('User Agent:', navigator.userAgent.substring(0, 80) + '...');
        console.log('Storage Status:', {
            localStorage: StorageUtil.isLocalStorageAvailable ? '✅' : '❌',
            sessionStorage: StorageUtil.isSessionStorageAvailable ? '✅' : '❌',
            memoryFallback: '✅'
        });
        
        // Show Android banner if data isn't loading properly
        const hasProjects = StorageUtil.getJSON('portfolioProjects');
        if (!hasProjects || (Array.isArray(hasProjects) && hasProjects.length === 0)) {
            // Show banner after a short delay to let page load
            setTimeout(() => {
                const banner = document.getElementById('android-banner');
                if (banner) {
                    banner.style.display = 'block';
                    console.warn('⚠️ No portfolio data found - banner displayed');
                }
            }, 1500);
        }
    }
